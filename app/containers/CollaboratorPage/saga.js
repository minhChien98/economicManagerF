...
export function* getAllItem(action) {
  try {
    const data = yield call(
      request,
      `${API_COLLABORATORS}?limit=${action.body.limit || 10}&skip=${action.body
        .skip || 0}`,
      {
        method: 'GET',
      },
...
export function* createNewRecord(action) {
  try {
    const data = yield call(request, `${API_COLLABORATORS}`, {
      method: 'POST',
      headers: {
    ...
    if (data) {
      ...
      const data2 = yield call(
        request,
        `${API_COLLABORATORS}?limit=${action.body.limit || 10}&skip=${action
          .body.skip || 0}`,
        {
          method: 'GET',
        },
...
export function* updateItem(action) {
  try {
    const data = yield call(request, `${API_COLLABORATORS}/${action.body.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
...
export default function* collaboratorPageSaga() {
