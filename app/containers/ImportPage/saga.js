...
import { API_IMPORT } from '../../utils/urlConfig';
...
export function* getAllItem(action) {
  try {
    const data = yield call(
      request,
      `${API_IMPORT}?limit=${action.body.limit || 10}&skip=${action.body.skip ||
        0}`,
      {
        method: 'GET',
      },
  ...
  try {
    ...
    if (data) {
      ...
      const data2 = yield call(
        request,
        `${API_IMPORT}?limit=${action.body.limit || 10}&skip=${action.body
          .skip || 0}`,
        {
          method: 'GET',
        },
...
export function* updateItem(action) {
  try {
    const data = yield call(request, `${API_IMPORT}/${action.body.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
