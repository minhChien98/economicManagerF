...
import { API_TIMEKEEPING } from '../../utils/urlConfig';
...
export function* getAllItem(action) {
  try {
    const data = yield call(
      request,
      `${API_TIMEKEEPING}?${action.body.params}`,
      {
        method: 'GET',
      },
    );
...
export function* createNewRecord(action) {
  try {
    const data = yield call(request, `${API_TIMEKEEPING}`, {
      method: 'POST',
      headers: {
...
export function* updateItem(action) {
  try {
    const data = yield call(request, `${API_TIMEKEEPING}/${action.body.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
