...
export function* getAllItem(action) {
  try {
    const data = yield call(
      request,
      `${API_EMPLOYEE}?limit=${action.body.limit || 10}&skip=${action.body
        .skip || 0}`,
      {
        method: 'GET',
      },
...
export function* createNewRecord(action) {
  try {
    const data = yield call(request, `${API_EMPLOYEE}`, {
      method: 'POST',
      headers: {
...
export function* updateItem(action) {
  try {
    const data = yield call(request, `${API_EMPLOYEE}/${action.body.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
