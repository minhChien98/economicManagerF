...
export function ExpensesPage(props) {
  useInjectReducer({ key: 'expensesPage', reducer });
  useInjectSaga({ key: 'expensesPage', saga });
  console.log(props)
  ...
  useEffect(() => {
    // When initial state username is not null, submit the form to load repos
    console.log('xxx')
    if (props.expensesPage.successCreate === true) {
      setOpenAdd(false);
      setOpenDelete(false);
      setItemSelected(null);
      props.onResetNoti();
    }
  }, [props.expensesPage.successCreate]);

  useEffect(() => {
    // When initial state username is not null, submit the form to load repos
    if (
      props.expensesPage.recordList &&
      props.expensesPage.recordList.length > 0
    ) {
      setData(props.expensesPage.recordList);
      setTotalCount(props.expensesPage.count);
    }
  }, [props.expensesPage.recordList]);
    ...
    <div>
      ...
      <Grid item md={12} sm={12} container>
        <Paper style={{ padding: '20px', width: '100%' }}>
          <Grid item md={12} sm={12} container>
            <Grid item md={2} sm={2}>
              <Typography variant="h6">Danh sách thu chi</Typography>
            </Grid>
            <Grid item md={3} sm={3}>
              <Fab
                color="primary"
          ...
          <Grid item md={12} sm={12} container>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell>Tên khoản chi</TableCell>
